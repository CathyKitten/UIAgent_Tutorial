...
Your task is to evaluate the quality of this instructional video. You need to rate each of the six criteria on a scale from 1 to 5 and provide a clear reason for each rating. The definitions and scoring guidelines for the six criteria are as follows:
...
"Repeated or unnecessary steps" refer to redundant actions in the instructional video that can be removed without affecting the completion of the task.
...
<3> Correctness
There are no incorrect steps in the instructional video. 
...
"Complete" means that the instructional video includes all the necessary steps required to transition an Excel task from an incomplete state to a completed state.
...
If a step like selecting the data is missing, the video is incomplete.
Scording Guide:
...
If the video shows applying the filter before selecting the data, the order is incorrect and can lead to confusion or failure to complete the task.
Scoring Guide:
...
The text and screenshots are well-aligned, with each screenshot accurately matching its corresponding textual description.
Good alignment means the screenshot directly supports what the text describes. For example, if the text says “Click the ‘Insert’ tab,” the screenshot should show the cursor pointing to the ‘Insert’ tab.
If the text specifies that a particular formula should be entered, but the screenshot does not show the actual formula, then the text and screenshot are considered mismatched.
Scoring Guide:
...
Here are the titles, descriptions, and screenshots for each step in the video:
  '''
                ...
                try:
                    ...
                    output_result_file = os.path.join(root, "video_judge_result_object_0721.json")
    ...
    for root, dirs, files in os.walk(root_directory):
        ...
        if 'video_step.json' in files:
            ...
            if os.path.basename(root) == 'video':
                ...
                task_overview=f'''## Task Overview
You are an evaluation system. You are provided with an Excel task request and the details for each step from the corresponding instructional video.
Your task is to evaluate the quality of this instructional video. You need to rate each of the five criteria on a scale from 1 to 5 and provide a clear reason for each rating. The definitions and scoring guidelines for the five criteria are as follows:
...
Here are the titles, descriptions, and screenshots for each step in the video:
  '''
                ...
                try:
                    result_json = json.loads(result_str)
                    output_result_file = os.path.join(root, "video_judge_result_subject_0721.json")
                    with open(output_result_file, "w", encoding="utf-8") as f:
                        json.dump(result_json, f, ensure_ascii=False, indent=2)
                    print(result_json)
